 ...
 * image, the polarizations are extracted from that image. Finally, all
 * extracted polarizations are resized with respect to
 ...
 */
public class SampleImageSegmenter implements ICapturedImageSetSegmenter {
    private final int _numChannels;
    private final IFieldOfView _fov;

    private final SampleImagePolarizationViewCreator _polViewCreator;
    private final PolarizationImageSetBuilder _polSetBuilder;

    public SampleImageSegmenter(IFieldOfView fov, int numChannels) {
        ...
        _polViewCreator = new SampleImagePolarizationViewCreator(_fov);
