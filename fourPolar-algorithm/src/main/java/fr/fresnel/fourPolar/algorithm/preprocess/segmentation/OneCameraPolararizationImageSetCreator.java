 ...
 */
class OneCameraPolararizationImageSetCreator extends PolarizationImageSetCreator {
    public OneCameraPolararizationImageSetCreator(IFieldOfView fov, int numChannels) {
    ...
    public void setCapturedImageSet(ICapturedImageSet capturedImageSet) {
        ...
        this._pol0 = this._segmenter.segment(capturedImages, pol0FoV);

        IBoxShape pol45FoV = this._fov.getFoV(Polarization.pol45);
        this._pol45 = this._segmenter.segment(capturedImages, pol45FoV);

        IBoxShape pol90FoV = this._fov.getFoV(Polarization.pol90);
        this._pol90 = this._segmenter.segment(capturedImages, pol90FoV);

        IBoxShape pol135FoV = this._fov.getFoV(Polarization.pol135);
        this._pol135 = this._segmenter.segment(capturedImages, pol135FoV);
