...
/**
 * Creates polarization view for registration images for the given channel. Note
 * that by a view we simply a cursor that is wrapped in an image. Hence the
 * process of creating a view does not add any memory overhead.
 * <p>
 * Each polarization view corresponds to the x and y dimension specified using
 * {@link IFieldOfView}. Note however that it will contain only the first z and
 * t points. Note finally that views are created regardless of whether or not
 * image is single channel or multi-channel.
 */
class RegistrationImagePolarizationViewCreator {
    private final IFieldOfView _fov;

    public RegistrationImagePolarizationViewCreator(IFieldOfView fov) {
        this._fov = fov;
    }
    ...
    private PolarizationView _createPolarizationView(ICapturedImage capturedImage, int channel, Polarization pol) {
        ...
        long[] viewLength = this._createViewLength(image.getMetadata(), _fov.getFoV(pol));
     ...
     * fov. If the channel axis is also not -1, it will be set as well. The z and t
     * axis if exist are set to 0 as well.
     * 
     * @return
     */
     ...
     * length would be the difference between min and max point of fov box. All
     * length of c, z and t are set to one, to get only a plane image.
     * 
     ...
     */
    private long[] _createViewLength(IMetadata capturedImageMetadata, IBoxShape polFoV) {
        ...
        if (capturedImageMetadata.axisOrder().z_axis > 0) {
            len[capturedImageMetadata.axisOrder().z_axis] = 1;
        }

        if (capturedImageMetadata.axisOrder().t_axis > 0) {
            len[capturedImageMetadata.axisOrder().t_axis] = 1;
        }

        if (capturedImageMetadata.axisOrder().c_axis > 0) {
