...
public class RegistrationMultiChannelPolarizationSegmenterTest {
    ...
    public void segment_OneMultiChannelXYCImage_ReturnsCorrectPolImages() {
        ...
        for (int i = 0; i < numChannels; i++) {
            ...
            assertTrue(BMCPSImageChecker._checkImage(image_pol45[i], 1, i + 1));
            assertTrue(BMCPSImageChecker._checkImage(image_pol90[i], 2, i + 1));
            assertTrue(BMCPSImageChecker._checkImage(image_pol135[i], 3, i + 1));
    ...
    public void segment_TwoMultiChannelXYCImage_ReturnsCorrectPolImages() {
        ...
        int numChannels = 6;
        ICapturedImage[] capturedImage = new ICapturedImage[] {
                new BMCPSDummyCapturedImage(AxisOrder.XYC, new long[] { 4, 4, numChannels / 2 }, new int[] { 1, 2, 3 }),
    ...
    public void segment_TwoMultiChannelXYZCTImage_ReturnsCorrectPolImages() {
        ...
        int numChannels = 6;
        ICapturedImage[] capturedImage = new ICapturedImage[] {
                new BMCPSDummyCapturedImage(AxisOrder.XYZCT, new long[] { 4, 4, 3, numChannels / 2, 1 },
                        new int[] { 1, 2, 3 }),
...
class BMCPSImageChecker {
    ...
    /**
     * Checks all values equal value the values of the first plane
     */
    public static boolean _checkImage(Image<UINT16> image, int value, int channel) {
        ...
        long[] planeDim = MetadataUtil.getPlaneDim(image.getMetadata()).point();
        long planeSize = planeDim[0] * planeDim[1];

        boolean equals = true;
        int pixelCounter = 0;
        for (IPixelCursor<UINT16> cursor = image.getCursor(); cursor.hasNext() && pixelCounter++ < planeSize
                && equals;) {
...
class BMCPSDummyCapturedImage implements ICapturedImage {
    Image<UINT16> image;
    IMetadata metadata;
    int[] channels;

    public BMCPSDummyCapturedImage(AxisOrder axisOrder, long[] dim, int[] channels) {
        ...
        BMCPSImageChecker._setImage(image, channels);
