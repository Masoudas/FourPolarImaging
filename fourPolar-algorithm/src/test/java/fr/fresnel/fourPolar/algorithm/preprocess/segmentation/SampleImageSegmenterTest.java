...
public class SampleImageSegmenterTest {
    @Test
    public void segment_OneSingleChannelXYImage_ReturnsCorrectPolImages() {
        ...
        ICapturedImage capturedImage = new SISDummyCapturedImage(capImage_axisOrder, capImage_dimension,
                capImage_channels);
        SISImageChecker._setImage(capturedImage.getImage(), capImage_channels, intersectionPoint);

        SISDummyCapturedImageSet capturedImageSet = new SISDummyCapturedImageSet(false);
        capturedImageSet.setFileSet(capturedImage);

        SampleImageSegmenter segmenter = new SampleImageSegmenter(fov, capImage_channels.length);
        IPolarizationImageSet pSet = segmenter.segment(capturedImageSet, capImage_channels[0]);

        _checkViews(pSet, fov, capImage_channels[0], 1, 1);
    ...
    public void segment_OneSingleChannelXYZImage_ReturnsCorrectPolImages() {
        ...
        _checkViews(polarizationImageSet, fov, capImage_channels[0], (int) capImage_dimension[2], 1);
    ...
    public void segment_OneTwoChannelXYCImage_ReturnsCorrectPolImages() {
        ...
        SISDummyCapturedImageSet capturedImageSet = new SISDummyCapturedImageSet(true);
        capturedImageSet.setFileSet(capturedImage);

        SampleImageSegmenter segmenter = new SampleImageSegmenter(fov, capImage_channels.length);
        IPolarizationImageSet polsetChannel1 = segmenter.segment(capturedImageSet, capImage_channels[0]);
        _checkViews(polsetChannel1, fov, capImage_channels[0], 1, 1);

        IPolarizationImageSet polsetChannel2 = segmenter.segment(capturedImageSet, capImage_channels[1]);
        _checkViews(polsetChannel2, fov, capImage_channels[1], 1, 1);
    }
    ...
    public void segment_OneTwoChannelXYCZTImage_ReturnsCorrectPolImages() {
        ...
        SampleImageSegmenter segmenter = new SampleImageSegmenter(fov, capImage_channels.length);
        IPolarizationImageSet polsetChannel1 = segmenter.segment(capturedImageSet, capImage_channels[0]);
        _checkViews(polsetChannel1, fov, capImage_channels[0], (int)capImage_dimension[3], (int)capImage_dimension[4]);

        IPolarizationImageSet polsetChannel2 = segmenter.segment(capturedImageSet, capImage_channels[1]);
        _checkViews(polsetChannel2, fov, capImage_channels[1], (int)capImage_dimension[3], (int)capImage_dimension[4]);
    }

    private void _checkViews(IPolarizationImageSet pSet, IFieldOfView fov, int channel, int dimz, int dimt) {
        // Dimension must equal maximum length of fovs in x and y dimension. Other
        // dimensions are 1.
        long[] dim = { fov.getMaximumLength()[0], fov.getMaximumLength()[1], 1, dimz, dimt };
        ...
        assertTrue(SISImageChecker._checkImage(pSet.getPolarizationImage(Polarization.pol0).getImage(), 0, channel));
        assertTrue(SISImageChecker._checkImage(pSet.getPolarizationImage(Polarization.pol45).getImage(), 1, channel));
        assertTrue(SISImageChecker._checkImage(pSet.getPolarizationImage(Polarization.pol90).getImage(), 2, channel));
        assertTrue(SISImageChecker._checkImage(pSet.getPolarizationImage(Polarization.pol135).getImage(), 3, channel));
    }
 ...
 * multichannel)
 */
class SISDummyCapturedImageSet implements ICapturedImageSet {
    ICapturedImage capturedImages;
    boolean isMultiChannel;

    public SISDummyCapturedImageSet(boolean isMultiChannel) {
...
class SISDummyCapturedImage implements ICapturedImage {
    Image<UINT16> image;
    IMetadata metadata;
    int[] channels;

    public SISDummyCapturedImage(AxisOrder axisOrder, long[] dim, int[] channels) {
...
class SISDummyFileSet implements ICapturedImageFileSet {
